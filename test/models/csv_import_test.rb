...
class CsvImportTest < ActiveSupport::TestCase
  ...
  test 'Should not update any attributes' do
    ...
    hash = {'reference': 2, 'email': 'test_1@test.com', 'home_phone_number': '2222222', 'mobile_phone_number': '3333333', 'firstname': 'Bruce', 'lastname': 'Wayne', 'address': 'Gotham'}
    CsvImport.update_blank_attribues(person, hash, Person::USER_UPDATABLE_ATTRIBUTES)
    assert_equal person, Person.find_by(reference: 1)
  end

  test 'Should update attributes' do
    person = Person.find_by(reference: 2)
    frozen_person = person.dup.freeze
    hash = {'reference': 2, 'email': 'test_2@test.com', 'home_phone_number': '3333333', 'mobile_phone_number': '4444444', 'firstname': 'Peter', 'lastname': 'Parker', 'address': 'New York City'}
    CsvImport.update_blank_attribues(person, hash, Person::USER_UPDATABLE_ATTRIBUTES)
  ...
  test 'Updatable person attributes should be' do
    ...
    assert_equal updatable_attributes, Person::USER_UPDATABLE_ATTRIBUTES
  end
